#!/bin/bash
#SBATCH --job-name=dickpt_bitmap_task_manual
#SBATCH --nodes=4
#SBATCH --ntasks=4
#SBATCH --ntasks-per-node=1
#SBATCH --time=00:30:00
#SBATCH --output=dickpt_bitmap_task_manual_%j.out
#SBATCH --error=dickpt_bitmap_task_manual_%j.err
#SBATCH --partition=compute

# Builds and runs the transpiler-generated OpenMP `task` test against the
# DICKPT bitmap monitor. The app (cape_task_manual.c) is produced by running
#   txl tests/test_task.c omptodickpt.Txl
...
#
# Pass criteria: the app prints "TASK_RESULT checksum=<C> expected=<E> ..."
# with C == E (the master scheduled all tasks, workers ran disjoint output
# ranges, and the merged/broadcast checkpoint reassembled the full result).
...
RESULTS_DIR="${RESULTS_DIR:-${SLURM_SUBMIT_DIR:-${PROJECT_DIR}}/results/dickpt_bitmap_task_manual_${JOB_TAG}}"
mkdir -p "${RESULTS_DIR}" 2>/dev/null || {
    RESULTS_DIR="/tmp/${USER}/dickpt_bitmap_task_manual_${JOB_TAG}"
    mkdir -p "${RESULTS_DIR}"
}

BUILD_DIR="${BUILD_DIR:-${SLURM_SUBMIT_DIR:-/tmp/${USER}}/cape_build_dickpt_task_manual_${JOB_TAG}}"
mkdir -p "${BUILD_DIR}/bin" "${BUILD_DIR}/obj" "${BUILD_DIR}/lib" 2>/dev/null || {
    BUILD_DIR="/tmp/${USER}/cape_build_dickpt_task_manual_${JOB_TAG}"
...
APP_SRC="${PROJECT_DIR}/src/apps/cape_task_manual.c"
if [ "${REGEN}" = "1" ]; then
    ...
    ( cd "${TXL_DIR}" && "${TXL_BIN}" tests/test_task.c omptodickpt.Txl ) > "${APP_SRC}.tmp" 2>/dev/null
...
make -C "${PROJECT_DIR}" dickpt_bitmap_monitor dickpt_task_manual PROFILE="${PROFILE}" "${MAKE_ARGS[@]}"

MONITOR="${BUILD_DIR}/bin/cape_dickpt_bitmap_monitor"
BIN="${BUILD_DIR}/bin/dickpt_task_manual"
TOTAL_NODES="${SLURM_JOB_NUM_NODES:-4}"

echo "Testing DICKPT bitmap OpenMP task (transpiler-generated manual)"
echo "App:     src/apps/cape_task_manual.c -> ${BIN}"
...
run_one() {
    local nn="$1"
    local tag="dickpt_bitmap_task_manual_nodes${nn}_reps${REPS}"
    ...
    local line
    line=$(grep -m1 "TASK_RESULT" "${log}" || true)
    if [ -z "${line}" ]; then
    ...
    local chk exp
    chk=$(sed -n 's/.*checksum=\([0-9-]*\).*/\1/p' <<<"${line}")
    exp=$(sed -n 's/.*expected=\([0-9-]*\).*/\1/p' <<<"${line}")
    if [ -n "${chk}" ] && [ "${chk}" = "${exp}" ]; then
        echo "[done]   ${tag}: PASS checksum=${chk}==expected"
    else
        echo "[fail]   ${tag}: checksum=${chk} expected=${exp} (${line})" >&2
        return 1
    fi
}
