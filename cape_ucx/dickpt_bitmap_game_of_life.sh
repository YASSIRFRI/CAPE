#!/bin/bash
#SBATCH --job-name=dickpt_bitmap_game_of_life
#SBATCH --nodes=64
#SBATCH --ntasks=64
#SBATCH --ntasks-per-node=1
#SBATCH --time=04:00:00
#SBATCH --output=dickpt_bitmap_game_of_life_%j.out
#SBATCH --error=dickpt_bitmap_game_of_life_%j.err
...
RESULTS_DIR="${RESULTS_DIR:-${SLURM_SUBMIT_DIR:-${PROJECT_DIR}}/results/dickpt_bitmap_game_of_life_${JOB_TAG}}"
mkdir -p "${RESULTS_DIR}" 2>/dev/null || { RESULTS_DIR="/tmp/${USER}/dickpt_bitmap_game_of_life_${JOB_TAG}"; mkdir -p "${RESULTS_DIR}"; }
BUILD_DIR="${BUILD_DIR:-${SLURM_SUBMIT_DIR:-/tmp/${USER}}/cape_build_dickpt_gol_${JOB_TAG}}"
mkdir -p "${BUILD_DIR}/bin" "${BUILD_DIR}/obj" "${BUILD_DIR}/lib" 2>/dev/null || \
  { BUILD_DIR="/tmp/${USER}/cape_build_dickpt_gol_${JOB_TAG}"; mkdir -p "${BUILD_DIR}/bin" "${BUILD_DIR}/obj" "${BUILD_DIR}/lib"; }
...
REPS="${REPS:-5}"
PROFILE="${PROFILE:-1}"
CAPE_UCX_SCRATCH_BYTES="${CAPE_UCX_SCRATCH_BYTES:-1073741824}"
...
make -C "${PROJECT_DIR}" dickpt_bitmap_monitor dickpt_game_of_life_manual PROFILE="${PROFILE}" "${MAKE_ARGS[@]}"

MONITOR="${BUILD_DIR}/bin/cape_dickpt_bitmap_monitor"
BIN="${BUILD_DIR}/bin/dickpt_game_of_life_manual"

CSV="${RESULTS_DIR}/dickpt_bitmap_game_of_life_summary_${JOB_TAG}.csv"
echo "impl,app,grid_bytes,live_per_rank,phases,nodes,rep,app_ms,job_id" > "${CSV}"

echo "Benchmarking DICKPT(bitmap) game_of_life"
echo "Monitor: src/monitor/cape_incr_bitmap.c -> ${MONITOR}"
echo "Nodes: ${NODES_LIST[*]}  Grid bytes: ${GRID_BYTES_LIST[*]}  Live/rank: ${LIVE_CELLS_LIST[*]}  Phases: ${PHASES_LIST[*]}  Reps: ${REPS}  MPI mode: ${SRUN_MPI_MODE}"
...
run_one() {
    ...
    local tag="dickpt_bitmap_game_of_life_g${grid_bytes}_l${live_cells}_p${phases}_nodes${nn}_rep${rep}"
    ...
    srun --exclusive --mpi="${SRUN_MPI_MODE}" --nodes="${nn}" --ntasks="${nn}" --ntasks-per-node=1 \
         "${MONITOR}" "${BIN}" "${grid_bytes}" "${live_cells}" "${phases}" 1 >>"${log}" 2>&1 || rc=$?
    rm -rf "${bdir}"
    if [ "${rc}" -ne 0 ]; then
        echo "[fail] ${tag} rc=${rc}" >&2; return
    fi
    awk -v impl="dickpt_bitmap" -v app="game_of_life" -v nn="${nn}" -v rep="${rep}" \
...
echo ""
echo "Done. DICKPT bitmap game_of_life summary CSV: ${CSV}"
