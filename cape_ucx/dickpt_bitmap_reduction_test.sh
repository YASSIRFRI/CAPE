#!/bin/bash
#SBATCH --job-name=dickpt_bitmap_reduction_test
#SBATCH --nodes=8
#SBATCH --ntasks=8
#SBATCH --ntasks-per-node=1
#SBATCH --time=00:30:00
#SBATCH --output=dickpt_bitmap_reduction_test_%j.out
#SBATCH --error=dickpt_bitmap_reduction_test_%j.err
...
RESULTS_DIR="${RESULTS_DIR:-${SLURM_SUBMIT_DIR:-${PROJECT_DIR}}/results/dickpt_bitmap_reduction_test_${JOB_TAG}}"
mkdir -p "${RESULTS_DIR}" 2>/dev/null || {
    RESULTS_DIR="/tmp/${USER}/dickpt_bitmap_reduction_test_${JOB_TAG}"
    mkdir -p "${RESULTS_DIR}"
}

BUILD_DIR="${BUILD_DIR:-${SLURM_SUBMIT_DIR:-/tmp/${USER}}/cape_build_dickpt_bitmap_reduction_${JOB_TAG}}"
mkdir -p "${BUILD_DIR}/bin" "${BUILD_DIR}/obj" "${BUILD_DIR}/lib" 2>/dev/null || {
    BUILD_DIR="/tmp/${USER}/cape_build_dickpt_bitmap_reduction_${JOB_TAG}"
...
make -C "${PROJECT_DIR}" dickpt_bitmap_monitor dickpt_reduction_manual PROFILE="${PROFILE}" "${MAKE_ARGS[@]}"

MONITOR="${BUILD_DIR}/bin/cape_dickpt_bitmap_monitor"
BIN="${BUILD_DIR}/bin/dickpt_reduction_manual"
TOTAL_NODES="${SLURM_JOB_NUM_NODES:-8}"

echo "Testing DICKPT bitmap reduction"
echo "Monitor: src/monitor/cape_incr_bitmap.c -> ${MONITOR}"
echo "App:     src/apps/cape_reduction_manual.c -> ${BIN}"
echo "Nodes: ${NODES_LIST[*]}  Reps: ${REPS}  MPI mode: ${SRUN_MPI_MODE}"
echo "Results dir: ${RESULTS_DIR}"

run_one() {
    local nn="$1"
    local tag="dickpt_bitmap_reduction_nodes${nn}_reps${REPS}"
    ...
    if ! grep -q "VERIFY OK" "${log}"; then
        echo "[fail] ${tag}: missing VERIFY OK in ${log}" >&2
        return 1
    fi
...
echo ""
echo "Done. DICKPT bitmap reduction logs in: ${RESULTS_DIR}"
