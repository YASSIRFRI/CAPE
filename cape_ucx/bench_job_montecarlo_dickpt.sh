...
# Monte Carlo Pi (embarrassingly parallel / reduction) — DICKPT.
# Sweeps nodes = 8,16,32,64,128, REPS runs each. CSV: impl,app,n,d,nodes,rep,app_ms,job_id

set -euo pipefail

APP="montecarlo"
TARGET="dickpt_montecarlo_manual"
BIN_NAME="dickpt_montecarlo_manual"
# Total samples drawn across all ranks.
N_SAMPLES="${N_SAMPLES:-2000000000}"
...
echo "Nodes: ${NODES_LIST[*]}  Reps: ${REPS}  samples=${N_SAMPLES}  MPI mode: ${SRUN_MPI_MODE}"
echo "CSV: ${CSV}"
