...
#ifdef USE_UCC_BCAST
/* =========================================================================
 * UCC integration — used for the master→workers broadcast.
 *
 * UCC needs an out-of-band (OOB) allgather to bootstrap a team. We
 * implement it on top of the existing UCX sendrecv: it's only invoked
 * during context/team setup (and possibly during destroy) so a
 * synchronous ring-allgather is fine. After that, broadcasts go through
 * UCC's optimized collective path (tree / pipeline depending on
 * transport — far better than N point-to-point sends from the master).
 * ========================================================================= */
#include <ucc/api/ucc.h>

static ucc_lib_h     ucc_lib;
static ucc_context_h ucc_ctx;
static ucc_team_h    ucc_team;

/* OOB allgather: each rank contributes msglen bytes from sbuf; on return
 * rbuf holds rank0's bytes, then rank1's, etc., concatenated. */
static ucc_status_t cape_ucc_oob_allgather(void *sbuf, void *rbuf,
                                           size_t msglen, void *coll_info,
                                           void **req)
{
    ...
    memcpy(r + (size_t)node * msglen, sbuf, msglen);

    /* Ring exchange to gather everyone else's contribution. */
    for (i = 1; i < num_nodes; ++i) {
        int dst = (int)((node + i) % num_nodes);
        int src = (int)((node + num_nodes - i) % num_nodes);
        cape_ucx_sendrecv(sbuf,                    msglen, dst,
                          r + (size_t)src * msglen, msglen, src,
                          0xC000u + (uint32_t)i);
    }

    /* Synchronous — completion tested via the test() / free() callbacks
     * which always return UCC_OK. The req handle is just a non-NULL
     * sentinel UCC will hand back to those callbacks. */
    *req = (void *)0x1;
    return UCC_OK;
}

static ucc_status_t cape_ucc_oob_req_test(void *req)
{ (void)req; return UCC_OK; }

static ucc_status_t cape_ucc_oob_req_free(void *req)
{ (void)req; return UCC_OK; }
...
static void cape_ucc_init(void)
{
    ...
    if (ucc_lib_config_read(NULL, NULL, &lib_cfg) != UCC_OK ||
        ucc_init(&lib_params, lib_cfg, &ucc_lib) != UCC_OK) {
        fprintf(stderr, "CAPE UCC: ucc_init failed\n");
        exit(1);
    }
    ucc_lib_config_release(lib_cfg);

    if (ucc_context_config_read(ucc_lib, NULL, &ctx_cfg) != UCC_OK) {
        fprintf(stderr, "CAPE UCC: ucc_context_config_read failed\n");
        exit(1);
    }

    memset(&ctx_params, 0, sizeof(ctx_params));
    ctx_params.mask              = UCC_CONTEXT_PARAM_FIELD_OOB;
    ctx_params.oob.allgather     = cape_ucc_oob_allgather;
    ctx_params.oob.req_test      = cape_ucc_oob_req_test;
    ctx_params.oob.req_free      = cape_ucc_oob_req_free;
    ctx_params.oob.coll_info     = NULL;
    ctx_params.oob.n_oob_eps     = (uint32_t)num_nodes;
    ctx_params.oob.oob_ep        = (uint32_t)node;

    if (ucc_context_create(ucc_lib, &ctx_params, ctx_cfg, &ucc_ctx)
            != UCC_OK) {
        fprintf(stderr, "CAPE UCC: ucc_context_create failed\n");
        exit(1);
    }
    ...
    team_params.oob            = ctx_params.oob;
    team_params.ep             = (uint64_t)node;
    team_params.ep_range       = UCC_COLLECTIVE_EP_RANGE_CONTIG;

    if (ucc_team_create_post(&ucc_ctx, 1, &team_params, &ucc_team)
            != UCC_OK) {
        fprintf(stderr, "CAPE UCC: ucc_team_create_post failed\n");
        exit(1);
    }
    while ((st = ucc_team_create_test(ucc_team)) == UCC_INPROGRESS)
        ucc_context_progress(ucc_ctx);
    if (st != UCC_OK) {
        fprintf(stderr, "CAPE UCC: ucc_team_create_test failed: %d\n", st);
        exit(1);
    }
}
...
static void cape_ucc_finalize(void)
{
    ucc_status_t st;
    if (ucc_team) {
        while ((st = ucc_team_destroy(ucc_team)) == UCC_INPROGRESS)
            ucc_context_progress(ucc_ctx);
        ucc_team = NULL;
    }
    if (ucc_ctx) {
...
/* Broadcast `bytes` of host memory at `buf` from `root` to all members
 * of the team. Synchronous: returns when the operation has completed
 * locally. */
static void cape_ucc_broadcast(void *buf, size_t bytes, int root)
{
    ucc_coll_args_t  args;
    ucc_coll_req_h   req;
    ucc_status_t     st;
    ...
    memset(&args, 0, sizeof(args));
    args.coll_type         = UCC_COLL_TYPE_BCAST;
    args.root              = (uint32_t)root;
    args.src.info.buffer   = buf;
    args.src.info.count    = bytes;
    args.src.info.datatype = UCC_DT_INT8;
    args.src.info.mem_type = UCC_MEMORY_TYPE_HOST;

    if (ucc_collective_init(&args, &req, ucc_team) != UCC_OK) {
        fprintf(stderr, "CAPE UCC: ucc_collective_init(BCAST) failed\n");
        exit(1);
    }
    if (ucc_collective_post(req) != UCC_OK) {
        fprintf(stderr, "CAPE UCC: ucc_collective_post(BCAST) failed\n");
        exit(1);
    }
    while ((st = ucc_collective_test(req)) == UCC_INPROGRESS)
        ucc_context_progress(ucc_ctx);
    if (st != UCC_OK) {
        fprintf(stderr, "CAPE UCC: ucc_collective_test(BCAST) failed: %d\n",
                st);
        exit(1);
    }
    ucc_collective_finalize(req);
