...
/* ------------------------------------------------------------------
 * Hybrid (multiple ranks per node) topology.
 *
 * Each SLURM task is an independent DICKPT rank (node/num_nodes come from
 * SLURM_PROCID/SLURM_NTASKS). When more than one rank is co-located on a
 * physical node we want the allreduce to move as few bytes as possible
 * over the slow inter-node link: reduce the local ranks over shared memory
 * to one leader per node, allreduce among leaders, then broadcast back
 * locally. UCX picks the shm transport for same-node peers automatically;
 * what we control here is the *algorithm* so the inter-node message count
 * scales with the number of nodes, not the number of ranks.
 *
 * Assumes SLURM block distribution (the default, and what the launch
 * scripts pin with --distribution=block:block): the local ranks of a node
 * occupy a contiguous global-rank range [node_base, node_base+local_size).
 * If that assumption or the env is missing we fall back to local_size==1,
 * i.e. the original flat allreduce, which is always correct.
 */
int cape_local_size = 1;   /* ranks per physical node */
int cape_local_rank = 0;   /* 0..cape_local_size-1 within this node */
int cape_node_base  = 0;   /* global rank of this node's leader (local_rank 0) */
int cape_n_leaders  = 1;   /* number of physical nodes = num_nodes/local_size */
...
/* Hybrid hierarchical allreduce: intra-node up/down + inter-node (leader) base */
#define TAG_HIER_UP        0x0A   /* local rank -> leader (intra-node reduce) */
#define TAG_HIER_DOWN      0x0B   /* leader -> local rank (intra-node bcast)  */
#define TAG_HIER_INTER_BASE 0x200 /* leader<->leader allreduce steps          */
...
int merge_external_checkpoint(FILE *src_ckpt_stream, 		\
							  ...
							  size_t src_ckpt_size 	)
 {
	...
	total_ckpt_size = 0;
 	...
 	//1. Read timespan directly from the buffers. This merge runs at every
 	//   allreduce hop on every iteration, so avoid two fmemopen()s (a heap
 	//   alloc + stdio object each) just to read 8 bytes; parse_bitmap_section
 	//   below likewise reads straight from the raw buffers.
	if (tmp_size < sizeof(unsigned long) ||
	    src_ckpt_size < sizeof(unsigned long)) {
		rc = 1;
		goto done;
	}
	memcpy(&t1, tmp_ckpt, sizeof(unsigned long));
	memcpy(&t2, src_ckpt_data, sizeof(unsigned long));
...
/* Resolve hybrid topology once, after node/num_nodes are known. Enables the
 * hierarchical allreduce only for a clean, contiguous, uniform block layout;
 * otherwise stays flat (local_size==1), which is always correct. */
static void cape_resolve_topology(void)
{
	int ls, lr;

	if (cape_topology_ready)
		return;
	cape_topology_ready = 1;

	cape_local_size = 1;
	cape_local_rank = 0;
	cape_node_base  = (int)node;
	cape_n_leaders  = num_nodes;
	cape_leader_id  = (int)node;

	lr = cape_env_int("SLURM_LOCALID", -1);
	...
	if (num_nodes % ls != 0 || lr >= ls)
		return;                 /* non-uniform / inconsistent -> stay flat */
	if (((int)node - lr) % ls != 0)
		return;                 /* not contiguous/block-distributed */

	cape_local_size = ls;
	cape_local_rank = lr;
	cape_node_base  = (int)node - lr;
	cape_n_leaders  = num_nodes / ls;
	cape_leader_id  = cape_node_base / ls;
	CAPE_DBG("hybrid topology: local_rank=%d local_size=%d node_base=%d "
		 "leader_id=%d n_leaders=%d",
		 cape_local_rank, cape_local_size, cape_node_base,
		 cape_leader_id, cape_n_leaders);
}

/* Merge a received buffer into the running total_ckpt (seeds it if empty). */
static int cape_merge_buffer(unsigned char *buf, size_t size)
{
	FILE *s;
	int rc;
	...
	if (buf == NULL || size == 0)
		return 1;
	s = fmemopen(buf, size, "rb");
	if (s == NULL)
		return 1;
	rc = merge_external_checkpoint(s, buf, size);
	fclose(s);
	return rc;
}

/* Inter-node allreduce over node leaders only. Virtual rank = cape_leader_id,
 * virtual size = cape_n_leaders; a leader's global rank is vrank*local_size
 * under the block-distribution assumption. Mirrors hypercube/ring but on the
 * leader sub-communicator. total_ckpt holds this node's reduced input on
 * entry and the global union on exit. */
static int cape_leader_allreduce(void)
{
	int rc = 0;
	FILE *ckpt_stream;
	...
	if (is_power_of_two((unsigned)cape_n_leaders)) {
		int nsteps = mylog2((unsigned)cape_n_leaders);
		int i;
		for (i = 0; i < nsteps; i++) {
			uint32_t token = TAG_HIER_INTER_BASE + (i * 2) + 1;
			int vpartner = cape_leader_id ^ (1 << i);
			int gpartner = vpartner * cape_local_size;
			size_t rsz;
			CAPE_PROFILE_NS_VAR(step_start_ns);
			CAPE_PROFILE_NS_START(step_start_ns);

			rsz = cape_ucx_sendrecv_probe(total_ckpt, total_ckpt_size,
						      gpartner, ucx_scratch_recv,
						      ucx_scratch_recv_cap,
						      gpartner, token);
			ckpt_stream = fmemopen(ucx_scratch_recv, rsz, "rb");
			if (ckpt_stream == NULL)
				return 1;
			rc = merge_external_checkpoint(ckpt_stream,
						       ucx_scratch_recv, rsz);
			fclose(ckpt_stream);
			if (rc != 0)
				return rc;
			CAPE_PROFILE_ADD_NS(allreduce_total_ns, step_start_ns);
		...
		unsigned char *send_buffer = total_ckpt;
		size_t message_size = total_ckpt_size;
		unsigned char *recv_buffer = ucx_scratch_recv;
		size_t recv_cap = ucx_scratch_recv_cap;
		int left  = (cape_leader_id - 1 + cape_n_leaders) % cape_n_leaders;
		int right = (cape_leader_id + 1) % cape_n_leaders;
		int gleft  = left  * cape_local_size;
		int gright = right * cape_local_size;
		int i;
		for (i = 1; i < cape_n_leaders; i++) {
			uint32_t token = TAG_HIER_INTER_BASE + (i * 2) + 1;
			size_t rsz;
			CAPE_PROFILE_NS_VAR(step_start_ns);
			CAPE_PROFILE_NS_START(step_start_ns);

			rsz = cape_ucx_sendrecv_probe(send_buffer, message_size,
						      gright, recv_buffer,
						      recv_cap, gleft, token);
			ckpt_stream = fmemopen(recv_buffer, rsz, "rb");
			if (ckpt_stream == NULL)
				return 1;
			rc = merge_external_checkpoint(ckpt_stream, recv_buffer, rsz);
			fclose(ckpt_stream);
			if (rc != 0)
				return rc;
			send_buffer = recv_buffer;
...
/* Hierarchical allreduce for hybrid (multiple ranks per node) runs:
 *   A. intra-node reduce  : local ranks -> leader (over shm)
 *   B. inter-node allreduce: leaders only (cape_leader_allreduce)
 *   C. intra-node bcast    : leader -> local ranks (over shm)
 * On entry every rank's total_ckpt holds its own delta; on exit every rank's
 * total_ckpt holds the global union, matching the flat allreduce result. The
 * bitmap merge is associative/commutative (disjoint-word union + reduction
 * ops), so grouping by node yields the same union. */
static int hierarchical_allreduce(void)
{
	int rc = 0;
	int p;

	/* Phase A: intra-node reduce to the node leader. */
	if (cape_local_rank != 0) {
		cape_ucx_send(total_ckpt, total_ckpt_size, cape_node_base,
			      TAG_HIER_UP);
	} else {
		for (p = 1; p < cape_local_size; p++) {
			int src = cape_node_base + p;
			size_t sz = 0;
			unsigned char *buf =
				cape_ucx_recv_probe_alloc(&sz, src, TAG_HIER_UP);
			rc = cape_merge_buffer(buf, sz);
			free(buf);
			if (rc != 0)
				return rc;
		}
	...
	/* Phase B: allreduce among leaders. */
	if (cape_local_rank == 0) {
		rc = cape_leader_allreduce();
		if (rc != 0)
			return rc;
	}

	/* Phase C: intra-node broadcast of the global union. */
	if (cape_local_rank == 0) {
		if (cape_total_stream_update_size() != 0)
			return 1;
		for (p = 1; p < cape_local_size; p++)
			cape_ucx_send(total_ckpt, total_ckpt_size,
				      cape_node_base + p, TAG_HIER_DOWN);
	} else {
		size_t sz = 0;
		unsigned char *buf;
		/* Drop the stale local delta so the received union seeds a
		 * fresh total_ckpt instead of merging on top of it. */
		cape_total_release();
		buf = cape_ucx_recv_probe_alloc(&sz, cape_node_base,
						TAG_HIER_DOWN);
		rc = cape_merge_buffer(buf, sz);
		free(buf);
	}
...
int require_allreduce_checkpoint(){
	...
	cape_resolve_topology();
	if (cape_local_size > 1)
		rc = hierarchical_allreduce();
	else if (is_power_of_two(num_nodes))
