#!/bin/bash
#SBATCH --job-name=dickpt_bitmap_critical_test
#SBATCH --nodes=4
#SBATCH --ntasks=4
#SBATCH --ntasks-per-node=1
#SBATCH --time=00:30:00
#SBATCH --output=dickpt_bitmap_critical_test_%j.out
#SBATCH --error=dickpt_bitmap_critical_test_%j.err
...
RESULTS_DIR="${RESULTS_DIR:-${SLURM_SUBMIT_DIR:-${PROJECT_DIR}}/results/dickpt_bitmap_critical_test_${JOB_TAG}}"
mkdir -p "${RESULTS_DIR}" 2>/dev/null || {
    RESULTS_DIR="/tmp/${USER}/dickpt_bitmap_critical_test_${JOB_TAG}"
    mkdir -p "${RESULTS_DIR}"
}

BUILD_DIR="${BUILD_DIR:-${SLURM_SUBMIT_DIR:-/tmp/${USER}}/cape_build_dickpt_bitmap_critical_${JOB_TAG}}"
mkdir -p "${BUILD_DIR}/bin" "${BUILD_DIR}/obj" "${BUILD_DIR}/lib" 2>/dev/null || {
    BUILD_DIR="/tmp/${USER}/cape_build_dickpt_bitmap_critical_${JOB_TAG}"
...
make -C "${PROJECT_DIR}" dickpt_bitmap_monitor dickpt_critical_manual PROFILE="${PROFILE}" "${MAKE_ARGS[@]}"

MONITOR="${BUILD_DIR}/bin/cape_dickpt_bitmap_monitor"
BIN="${BUILD_DIR}/bin/dickpt_critical_manual"
TOTAL_NODES="${SLURM_JOB_NUM_NODES:-4}"

echo "Testing DICKPT bitmap OpenMP critical (checkpoint-chained)"
echo "DICKPT:  src/apps/cape_critical_manual.c -> ${BIN}"
...
run_one() {
    local nn="$1"
    local tag="dickpt_bitmap_critical_nodes${nn}_reps${REPS}"
    ...
    if ! grep -q "CRITICAL_RESULT ok=1" "${log}"; then
        echo "[fail] ${tag}: missing CRITICAL_RESULT ok=1 in ${log}" >&2
        return 1
    fi
