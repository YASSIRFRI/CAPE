#!/bin/bash
#SBATCH --job-name=cape_verify_task
#SBATCH --nodes=4
#SBATCH --ntasks=4
#SBATCH --ntasks-per-node=1
#SBATCH --time=00:30:00
#SBATCH --output=cape_verify_task_%j.out
#SBATCH --error=cape_verify_task_%j.err
...
APP_SRC="${APP_SRC:-${PROJECT_DIR}/../20171001 TXL Transform/20171001 TXL Transform/openmptocape/verify_task_cape.c}"
if [ ! -f "${APP_SRC}" ]; then
...
RESULTS_DIR="${RESULTS_DIR:-${SLURM_SUBMIT_DIR:-${PROJECT_DIR}}/results/cape_verify_task_${JOB_TAG}}"
mkdir -p "${RESULTS_DIR}" 2>/dev/null || { RESULTS_DIR="/tmp/${USER}/cape_verify_task_${JOB_TAG}"; mkdir -p "${RESULTS_DIR}"; }
BUILD_DIR="${BUILD_DIR:-${SLURM_SUBMIT_DIR:-/tmp/${USER}}/cape_build_verify_task_${JOB_TAG}}"
mkdir -p "${BUILD_DIR}/bin" 2>/dev/null || { BUILD_DIR="/tmp/${USER}/cape_build_verify_task_${JOB_TAG}"; mkdir -p "${BUILD_DIR}/bin"; }
...
BIN="${BUILD_DIR}/bin/verify_task"
echo "Compiling verify_task (CAPE bitmap backend)"
echo "  app:     ${APP_SRC}"
...
run_one() {
    local nn="$1" rep="$2"
    local tag="cape_verify_task_nodes${nn}_rep${rep}"
    ...
    if grep -q "status=FAIL" "${log}"; then
        echo "[fail] ${tag}: task verification failed (see ${log})" >&2
        return 1
    fi
...
echo ""
echo "Done. verify_task logs in: ${RESULTS_DIR}"
