...
for nn in "${NODES_LIST[@]}"; do
    for n in "${N_MAMULT[@]}";  do for r in $(seq 1 ${REPS}); do run_one mul_manual "${BUILD_DIR}/bin/mpi_mul_manual" "${n}" ""  "${nn}" "${r}"; done; done
    for n in "${N_MATVEC[@]}";  do for r in $(seq 1 ${REPS}); do run_one matvec     "${BUILD_DIR}/bin/mpi_matvec"     "${n}" ""  "${nn}" "${r}"; done; done
    for pair in "${GRADIENT_PAIRS[@]}"; do
        n="${pair%:*}"; d="${pair#*:}"
        for r in $(seq 1 ${REPS}); do run_one gradient "${BUILD_DIR}/bin/mpi_gradient" "${n}" "${d}" "${nn}" "${r}"; done
    done
done
