#!/bin/bash
#SBATCH --job-name=bench_nbody_dickpt
#SBATCH --qos=himem-cpu
#SBATCH --nodes=128
#SBATCH --ntasks=128
#SBATCH --ntasks-per-node=1
#SBATCH --time=04:00:00
#SBATCH --output=bench_nbody_dickpt_%j.out
#SBATCH --error=bench_nbody_dickpt_%j.err
#SBATCH --partition=compute

# N-Body particle simulation (all-to-all) — DICKPT.
# Sweeps nodes = 8,16,32,64,128, REPS runs each. CSV: impl,app,n,d,nodes,rep,app_ms,job_id

set -euo pipefail

APP="nbody"
TARGET="dickpt_nbody_manual"
BIN_NAME="dickpt_nbody_manual"
...
echo "Nodes: ${NODES_LIST[*]}  Reps: ${REPS}  particles=${N_PARTICLES} steps=${N_STEPS}  MPI mode: ${SRUN_MPI_MODE}"
echo "CSV: ${CSV}"
...
run_one() {
    ...
    srun --exclusive --mpi="${SRUN_MPI_MODE}" --nodes="${nn}" --ntasks="${nn}" --ntasks-per-node=1 \
         "${MONITOR}" "${BIN}" "${N_PARTICLES}" "${N_STEPS}" "${REPS}" >>"${log}" 2>&1 || rc=$?
    rm -rf "${bdir}"
