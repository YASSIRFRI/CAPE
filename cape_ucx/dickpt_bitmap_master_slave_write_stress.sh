#!/bin/bash
#SBATCH --job-name=dickpt_bitmap_ms_write_stress
#SBATCH --nodes=32
#SBATCH --ntasks=32
#SBATCH --ntasks-per-node=1
#SBATCH --time=04:00:00
#SBATCH --output=dickpt_bitmap_ms_write_stress_%j.out
#SBATCH --error=dickpt_bitmap_ms_write_stress_%j.err
...
RESULTS_DIR="${RESULTS_DIR:-${SLURM_SUBMIT_DIR:-${PROJECT_DIR}}/results/dickpt_bitmap_ms_write_stress_${JOB_TAG}}"
mkdir -p "${RESULTS_DIR}" 2>/dev/null || { RESULTS_DIR="/tmp/${USER}/dickpt_bitmap_ms_write_stress_${JOB_TAG}"; mkdir -p "${RESULTS_DIR}"; }
BUILD_DIR="${BUILD_DIR:-${SLURM_SUBMIT_DIR:-/tmp/${USER}}/cape_build_dickpt_bms_${JOB_TAG}}"
mkdir -p "${BUILD_DIR}/bin" "${BUILD_DIR}/obj" "${BUILD_DIR}/lib" 2>/dev/null || \
  { BUILD_DIR="/tmp/${USER}/cape_build_dickpt_bms_${JOB_TAG}"; mkdir -p "${BUILD_DIR}/bin" "${BUILD_DIR}/obj" "${BUILD_DIR}/lib"; }
...
# Tunable: cap on the master's in-flight unmerged ckpt payloads (bytes).
# Higher → more parallel ingress, more peak RAM. Lower → tighter back-pressure.
CAPE_MS_MAX_QUEUE_BYTES="${CAPE_MS_MAX_QUEUE_BYTES:-268435456}"  # 256 MiB
...
elif pkg-config --exists ucc 2>/dev/null; then
    UCC_INC="$(pkg-config --variable=includedir ucc)"
    UCC_LIB="$(pkg-config --variable=libdir ucc)"
else
    echo "ERROR: cannot locate UCC installation. Load a UCC module or set EBROOTUCC/PKG_CONFIG_PATH." >&2
    exit 1
fi
...
# Inject the queue-size cap as a compile-time define.
PMIX_FLAGS="${PMIX_FLAGS} -DCAPE_MS_MAX_QUEUE_BYTES=${CAPE_MS_MAX_QUEUE_BYTES}ull"
...
make -C "${PROJECT_DIR}" dickpt_bitmap_master_slave_monitor dickpt_write_stress PROFILE="${PROFILE}" "${MAKE_ARGS[@]}"

MONITOR="${BUILD_DIR}/bin/cape_dickpt_bitmap_master_slave_monitor"
BIN="${BUILD_DIR}/bin/dickpt_write_stress"

CSV="${RESULTS_DIR}/dickpt_bitmap_ms_write_stress_summary_${JOB_TAG}.csv"
echo "impl,app,n,phases,nodes,rep,app_ms,job_id" > "${CSV}"

echo "Benchmarking DICKPT(bitmap, master-slave) write_stress"
echo "Nodes: ${NODES_LIST[*]}  N: ${N_LIST[*]}  Phases: ${PHASES_LIST[*]}  Reps: ${REPS}  MPI mode: ${SRUN_MPI_MODE}"
echo "Master queue cap: ${CAPE_MS_MAX_QUEUE_BYTES} bytes"
...
run_one() {
    ...
    local tag="dickpt_bitmap_ms_write_stress_n${n}_p${phases}_nodes${nn}_rep${rep}"
    ...
    awk -v impl="dickpt_bitmap_ms" -v app="write_stress" -v nn="${nn}" -v rep="${rep}" \
...
echo ""
echo "Done. DICKPT bitmap master-slave write_stress summary CSV: ${CSV}"
