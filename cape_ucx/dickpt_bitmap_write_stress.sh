#!/bin/bash
#SBATCH --job-name=dickpt_bitmap_write_stress
#SBATCH --nodes=32
#SBATCH --ntasks=32
#SBATCH --ntasks-per-node=1
#SBATCH --time=04:00:00
#SBATCH --output=dickpt_bitmap_write_stress_%j.out
#SBATCH --error=dickpt_bitmap_write_stress_%j.err
...
RESULTS_DIR="${RESULTS_DIR:-${SLURM_SUBMIT_DIR:-${PROJECT_DIR}}/results/dickpt_bitmap_write_stress_${JOB_TAG}}"
mkdir -p "${RESULTS_DIR}" 2>/dev/null || { RESULTS_DIR="/tmp/${USER}/dickpt_bitmap_write_stress_${JOB_TAG}"; mkdir -p "${RESULTS_DIR}"; }
BUILD_DIR="${BUILD_DIR:-${SLURM_SUBMIT_DIR:-/tmp/${USER}}/cape_build_dickpt_bws_${JOB_TAG}}"
mkdir -p "${BUILD_DIR}/bin" "${BUILD_DIR}/obj" "${BUILD_DIR}/lib" 2>/dev/null || \
  { BUILD_DIR="/tmp/${USER}/cape_build_dickpt_bws_${JOB_TAG}"; mkdir -p "${BUILD_DIR}/bin" "${BUILD_DIR}/obj" "${BUILD_DIR}/lib"; }
...
make -C "${PROJECT_DIR}" dickpt_bitmap_monitor dickpt_write_stress PROFILE="${PROFILE}" "${MAKE_ARGS[@]}"

MONITOR="${BUILD_DIR}/bin/cape_dickpt_bitmap_monitor"
BIN="${BUILD_DIR}/bin/dickpt_write_stress"

CSV="${RESULTS_DIR}/dickpt_bitmap_write_stress_summary_${JOB_TAG}.csv"
echo "impl,app,n,phases,nodes,rep,app_ms,job_id" > "${CSV}"

echo "Benchmarking DICKPT(bitmap) write_stress"
...
run_one() {
    ...
    local tag="dickpt_bitmap_write_stress_n${n}_p${phases}_nodes${nn}_rep${rep}"
    ...
    awk -v impl="dickpt_bitmap" -v app="write_stress" -v nn="${nn}" -v rep="${rep}" \
...
echo ""
echo "Done. DICKPT bitmap write_stress summary CSV: ${CSV}"
